...
const lbcf2 = [];
chapters.forEach(c => { 
  for(let p=1;p<=c.p;p++) lbcf2.push({src:'2LBCF',reading:`Ch. ${c.ch} §${p}`,detail:c.t}) 
});
...
const lbcf1 = lbcf1Titles.map((t,i)=>({
  src:'1LBCF',
  reading:`Article ${i+1}`,
  detail:t
}));
  ...
  for (let day=1; day<=365; day++) {
    if (day % 7 === 0) {
      pool.push({
        ...
        detail:reviewPrompts[iR%reviewPrompts.length]
      });
      iR++;
    } else {
      const turn = (day - Math.floor(day/7)) % 3;
      let pushed = false;
      const tryPush = (arr, idx) => { 
        if(idx < arr.length){
          pool.push({day,date:dateStr(day),...arr[idx]});
          return true;
        }
        return false; 
      };
...
// Generate CSV
const csvHeader = 'Day,Date,Source,Reading,Detail\n';
const csvLines = schedule.map(e => 
  `${e.day},"${e.date}","${e.src}","${e.reading}","${e.detail.replace(/"/g, '""')}"`
).join('\n');
const csvContent = csvHeader + csvLines;
fs.writeFileSync('schedule.csv', csvContent);

// Generate Text file with better formatting
const txtLines = schedule.map(e => 
  `Day ${String(e.day).padStart(3)} | ${e.date.padEnd(10)} | ${e.src.padEnd(10)} | ${e.reading.padEnd(20)} | ${e.detail}`
).join('\n');
const txtHeader = 'Particular Baptist Devotional - 365-Day Schedule\n' +
  '=====================================================\n\n';
fs.writeFileSync('schedule.txt', txtHeader + txtLines);

console.log('✓ schedule.csv generated (365 days)');
console.log('✓ schedule.txt generated (365 days)');
