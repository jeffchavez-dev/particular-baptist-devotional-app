...
const lbcf2 = [];
chapters.forEach(c => { for(let p=1;p<=c.p;p++) lbcf2.push({src:'2LBCF',reading:`Ch. ${c.ch} §${p}`,detail:c.t}) });

const catechism = Array.from({length:114},(_,i)=>({src:'Catechism',reading:`Q&A #${i+1}`,detail:'The Baptist Catechism (Keach\'s)'}));
...
const lbcf1 = lbcf1Titles.map((t,i)=>({src:'1LBCF',reading:`Article ${i+1}`,detail:t}));
...
function buildSchedule() {
  ...
  for (let day=1; day<=365; day++) {
    if (day % 7 === 0) {
      pool.push({day, date:dateStr(day), src:'Review', reading:'Weekly review & reflection', detail:reviewPrompts[iR%reviewPrompts.length]});
      iR++;
    } else {
...
schedule.forEach(entry => {
  output += `Day ${String(entry.day).padStart(3)} (${entry.date}): ${entry.src.padEnd(10)} - ${entry.reading.padEnd(15)} | ${entry.detail}\n`;
});
fs.writeFileSync('schedule.txt', output);
console.log('Schedule written to schedule.txt (365 days)');
